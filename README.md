...
4. The OS fetches `http://<serverIP>/ignition.json?mac=<mac>`; Booty renders the host's Butane template (variables: `.Hostname`, `.ServerIP`, `.JoinString`, `.OSTreeImage`) and translates it to Ignition. Unregistered hosts receive an Ignition config whose only unit reboots the machine (the "brig").
