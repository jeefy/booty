...
**Every credential is encrypted with the systemd "null" key**, exactly as `systemd-creds --with-key=null encrypt --name=<name>` would write it (AES-256-GCM under a fixed key, base64 text). This is not for secrecy -- the null key is public -- but because systemd only imports `/loader/credentials/*.cred` as *encrypted* credentials (`/run/credentials/@encrypted`); plaintext files there make every `ImportCredential=` consumer fail with `Failed to set up credentials: Invalid argument`. Booty does this in Go, no `systemd-creds` binary needed at run time, and derives the IV from the credential name and contents so the bundle (and its digest) stays stable across restarts. The name embedded in each credential equals its file name without `.cred`, which systemd checks. Caveat inherited from systemd: a machine with a TPM2 *and* Secure Boot enabled refuses null-key credentials (`Credential uses fixed key for fallback use when TPM2 is absent -- but TPM2 is present, and SecureBoot is enabled, refusing`); disable Secure Boot on such hosts or provision them another way.
...
* `tmpfiles.extra.cred` (`sshkeys`, `update`) -- `systemd-tmpfiles` rules for files only: `/home/core/.ssh/authorized_keys` from `--sshAuthorizedKeys`/`--sshAuthorizedKeysFile` (the `core` user exists and SSH is key-only, so without this nobody can log in) and `/opt/booty/update-check`.
* `systemd.extra-unit.booty-booted.service.cred` (`booted`), `systemd.extra-unit.booty-update.service.cred` and `systemd.extra-unit.booty-update.timer.cred` (`update`) -- the unit files, the same text the Ignition builtin ships, delivered through [systemd-debug-generator(8)](https://www.freedesktop.org/software/systemd/man/latest/systemd-debug-generator.html). Units written under `/etc/systemd/system` by tmpfiles are never loaded (tmpfiles runs after PID 1 has read the unit tree), which is why they are not in `tmpfiles.extra`.
* `systemd.unit-dropin.multi-user.target~booty.cred` (`booted`) and `systemd.unit-dropin.timers.target~booty.cred` (`update`) -- `[Unit] Wants=` drop-ins that pull the units in. The generator ignores `[Install]`, so these do what `systemctl enable` would have done.
