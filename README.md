...
* `--clusterDistribution=k0s`: the same with k0s -- a controller and workers on Flatcar, Fedora CoreOS **and Bluefin Server**, whose shipped `k0s` is retargeted through the credentials bundle; workers get a one-hour join token per boot, minted like the kubeadm ones (see [Managed k0s](#managed-k0s))
...
A k0s join token is a kubeconfig wrapping a Kubernetes bootstrap-token Secret -- the same object `--kubeadmJoin=auto` creates -- so Booty mints k0s worker tokens with the same `Minter`, no flag needed. When a `flatcar`/`coreos` worker fetches `/ignition/builtin.json` (the real child fetch of a boot, not a `preview=1`), or a `bluefin` worker's install stanza and credentials bundle are rendered, Booty
...
2. encodes it as `k0s token pre-shared` would (`kubelet-bootstrap` user, `https://<endpoint>:6443`, the cluster CA) and writes it to `/etc/k0s/token`, caching it per MAC for `ttl/2` so the retries of one boot -- and, for Bluefin, the `/booty.ipxe` render that embeds `inst.creds_sha256` and the `/creds/<mac>.tar` download seconds later -- see the same bytes.
