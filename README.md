...
* `--clusterDistribution=k0s`: the same with k0s -- a controller and workers on Flatcar, Fedora CoreOS **and Bluefin Server**, whose shipped `k0s` is retargeted through the credentials bundle (see [k0s](#k0s))
...
Booty is growing from "join workers to a cluster you already have" into provisioning a whole cluster from PXE: a Booty-managed or external control plane, `kubeadm` or `k0s`, on Flatcar, Fedora CoreOS and Bluefin Server, with the CNI installed from the first control plane. The design and the slices it ships in are in [docs/plans/2026-09-26-cluster-bootstrap.md](docs/plans/2026-09-26-cluster-bootstrap.md). Shipped so far: the model, CA and tokens (H1), the managed **kubeadm** control plane with its CNI (H2) and **k0s** on every OS including the Bluefin controller (H3). The defaults (`kubeadm`, `external`) equal the join-only behaviour above, and a host without a `role` renders byte-identical to before -- except for the two deliberate worker changes below -- guarded by a golden test.

| `--clusterDistribution` | Flatcar | Fedora CoreOS | Bluefin Server |
|---|---|---|---|
| `kubeadm` (default) | control plane + worker | control plane + worker | not supported (HTTP 400 under `managed`) |
| `k0s` | controller + worker | controller + worker | controller + worker (via the credentials bundle) |
...
**Give PXE-booted nodes a `--containerdDisk` too.** A Flatcar or CoreOS node booted from the network keeps its root filesystem in RAM (a tmpfs of half the memory), and a Kubernetes node's images do not fit there: in QEMU a 3 GiB worker filled its 1.5 GiB root to 96 % with Cilium's images and went `ImagePullBackOff`, and the control plane's containerd competes with etcd for the same space. With `--containerdDisk` the images live on a disk and 3 GiB workers come up `Ready`; without it plan on roughly 6 GiB of RAM per node or more.
...
| `/etc/k0s/token` (0600) | -- | The pre-shared worker join token (`k0s token pre-shared` format: a gzip+base64 kubeconfig with Booty's CA, `https://<endpoint>:6443` and the bootstrap token). With `--controlPlane=external` it is the contents of `--k0sTokenFile`, which must be a *worker* token. |
...
**Flatcar and Fedora CoreOS** get, on top of the builtin fragment, `booty-k0s-install.service` (`k0s-install.sh`: downloads the `--k0sVersion` amd64 binary from the [k0s release](https://github.com/k0sproject/k0s/releases) to `/opt/bin/k0s` and verifies its sha256 -- the default `v1.36.4+k0s.0` is pinned in code (`ca1e9e68107335846e8296777fce2ccd654284e6265b4b5d32c34ead872af98f`, checked against the release's `sha256sums.txt` and GitHub's asset digest on 2026-09-26), any other version is verified against that release's `sha256sums.txt`; a binary that already matches is left alone) and then either `k0scontroller.service` (`/opt/bin/k0s controller -c /etc/k0s/k0s.yaml --enable-worker --disable-components=helm,autopilot`) or `k0sworker.service` (`/opt/bin/k0s worker --token-file /etc/k0s/token`), both shaped like the unit `k0s install` writes (`Restart=always`, `Delegate=yes`, `KillMode=process`) and ordered after the install oneshot and the `/var/lib/k0s` mount. Both OSes run from RAM when PXE-booted, so the same two disks as under kubeadm apply: a **controller requires `--controlPlaneDisk`** (mounted at `/var/lib/booty-cp`, `/var/lib/k0s` bind-mounted from `/var/lib/booty-cp/k0s` after `booty-cp-seed.service` copied the Ignition-written PKI and manifests onto it with `cp -an`; `manifests/booty/` is refreshed on every boot so a rebooted controller applies the token Secret Booty currently renders) and puts `--containerdDisk`, when given, at `/var/lib/k0s/containerd`; a **worker** puts the whole `/var/lib/k0s` (k0s's extracted binaries, containerd and kubelet state) on `--containerdDisk` and re-joins with its token on every boot. Rendering a Flatcar/CoreOS controller without `--controlPlaneDisk` is the same HTTP 400 as under kubeadm. Like k0s itself does for controller+worker nodes, the controller keeps the `node-role.kubernetes.io/master:NoExecute` taint.

**Bluefin Server** already ships `/usr/bin/k0s` (`v1.36.4+k0s.0` in 26.08.0, the same as `--k0sVersion`'s default) and a `k0scontroller.service` whose stock command is `k0s controller --enable-worker --single --disable-components=helm,autopilot`; its `k0s-first-boot.service` runs on every boot and ends with `systemctl enable --now k0scontroller.service`. Booty does not fight that: the [credentials bundle](#credentials-bundle) carries the node's k0s files inside `tmpfiles.extra` (still exactly two credentials) plus a drop-in `/etc/systemd/system/k0scontroller.service.d/booty-role.conf` that resets `ExecStart=` and sets it to `/usr/bin/k0s controller -c /etc/k0s/k0s.yaml --enable-worker --disable-components=helm,autopilot` on a controller (dropping `--single`, which is incompatible with joins and etcd) or `/usr/bin/k0s worker --token-file /etc/k0s/token` on a worker. **A Bluefin worker therefore runs a unit called `k0scontroller.service`** -- renaming it would break `k0s-first-boot`'s `enable --now` by name. The existing `booty.conf` drop-in (`Wants=`) also pulls `booty-cluster-ready.service` and `booty-cni-apply.service` in on the first boot. Bluefin needs neither `--controlPlaneDisk` nor `--containerdDisk` (it is installed to disk). The image's `k0s-first-boot` also seeds `/var/lib/k0s/manifests/{argocd,kubestellar}` on every boot: a Booty-managed Bluefin controller applies those appliance manifests too (Argo CD and KubeStellar land in the cluster), and Booty leaves that alone; on a worker they are inert.

The k0s worker token is the persisted 7-day bootstrap token from `cluster/tokens.json` (renewed when under 24 h remain), and the Secret making it valid is applied by the controller from its manifests directory. Booty does not yet renew that Secret through the API (there is no k0s equivalent of `--kubeadmJoin=auto`), so once the token has rotated, PXE workers that re-join on boot need the controller to have rebooted (Flatcar/CoreOS re-seed the manifest) or the Secret re-applied by hand; Bluefin workers that already joined keep their kubelet certificate and never use the token again. `--kubeadmJoin`, `--joinString` and the `Minter` do nothing under k0s: no kubeadm token is minted and no `X-Booty-Warning` is raised; `{{ .JoinString }}` is only ever the static `--joinString` there.
...
* `tmpfiles.extra.cred` (`hostname`, `sshkeys`, `booted`, `update`) -- `systemd-tmpfiles` rules for everything else:
  ...
  * under `--clusterDistribution=k0s` (any enabled feature): the host's k0s pieces from [k0s](#k0s) -- `/etc/k0s/k0s.yaml`, the six PKI files and the token-Secret manifest on a controller, `/etc/k0s/token` on a worker, the `booty-cluster-ready`/`booty-cni-apply` units, and the `k0scontroller.service.d/booty-role.conf` drop-in that swaps the image's `ExecStart=` for Booty's controller or worker command.
