...
* `--clusterDistribution=k0s`: the same with k0s -- a controller and workers on Flatcar, Fedora CoreOS **and Bluefin Server**, whose shipped `k0s` is retargeted through the credentials bundle (see [Managed k0s](#managed-k0s))
...
**Give PXE-booted nodes a `--containerdDisk` too.** A network-booted node's images do not fit its RAM root: in QEMU a 3 GiB worker filled its 1.5 GiB root to 96 % with Cilium's images and went `ImagePullBackOff`, and the control plane's containerd competes with etcd for the same space. With `--containerdDisk` the images live on a disk (ext4, label `ssd`, wiped on every boot: at `/var/lib/containerd` under kubeadm, at `/var/lib/k0s/containerd` on a k0s controller and at `/var/lib/k0s` itself on a k0s worker, whose extracted binaries and kubelet state also live there) and 3 GiB workers come up `Ready`; without it plan on roughly 6 GiB of RAM per node or more. Bluefin installs to disk and is exempt from both flags.
...
* `tmpfiles.extra.cred` (`hostname`, `sshkeys`, `booted`, `update`) -- `systemd-tmpfiles` rules for everything else:
  ...
  * under `--clusterDistribution=k0s` (any enabled feature): the host's k0s pieces from [Managed k0s](#managed-k0s) -- `/etc/k0s/k0s.yaml`, the six PKI files and the token-Secret manifest on a controller, `/etc/k0s/token` on a worker, the `booty-cluster-ready`/`booty-cni-apply` units, and the `k0scontroller.service.d/booty-role.conf` drop-in that swaps the image's `ExecStart=` for Booty's controller or worker command.
