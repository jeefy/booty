...
**The control-plane host** (`role: control-plane`, `os: flatcar` or `coreos`) gets, on top of the builtin fragment, the same tool chain as a worker (`booty-cni-install` -> `booty-kube-tools` -> `booty-kubelet-setup`) and then:
